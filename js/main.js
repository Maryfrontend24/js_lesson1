// вывод информации -фУНКЦИЯ ВЫВОДА
console.log("1111")
alert("dfdfff")
...
let age 
// Присвоение - раньше писали var (тоже самое, что  - let )
age = "12 лет"
...
// НЕ ЦЕЛЫЕ ЧИСЛА ПИЩУТСЯ ЧЕРЕЗ ТОЧКУ - 5.7 , 8.1     !!!!!!!!!!!!!!!!!!!!


// b = b+1 Или b+=1 (увеличение на 1)   ++b (увеличение на 1 )  --b !!!!!!!

// Конкатенация -соединение двух или более строк в одну!!!!!!!!!

// length - свойство, которое позволяет узнать длину строки

let name = "Maria"
// "Maria".length

// name[name.length - 1] - узнать последний знак строки

                                    // Методы
// Метод slice!!!!!!!!!!!!!!!!

// name.tolowerCase() превращает все символы в нижний регистр!!!!
// name.toUpperCase()  превращает все символы в верхний регистр (Заглавные)!!!!
let text ="  СТроКа  "
text.toLowerCase()
" строка "
...
Получаем - "Строка"
// оТСЧЕТ ИНДЕКСОВ В СТРОКЕ ИДЕТ С 0 (0123456789....)


                                //    ЛОГИЧЕСКИЙ ТИП ДАННЫХ

                                // МОЖЕТ СОДЕРЖАТЬ  ЗНАЧЕНИЯ  TRUE AND FALSE!!!!!!!!!!!!

//                                 С НИМИ МОЖНО ВЫПОЛНЯТЬ : ЛОГИЧЕСКОЕ "и"
// "ИЛИ"   "НЕ" (ОТРИЦАНИЕ)!!!!!!!!!!!!!!!!!!!!!!!!!!!!

// Значение null - переменная пустая ( a =  null)
// let a;
// undefined - значение переменной не определено 


// !"use strict" -строгий режим ()ЧТОБЫ СТАРЫЙ КОД АДЕКВАТНО РАБОТАЛ-НОВЫЕ ФУНКЦИИ ПО УМОЛЧАНИЮ ВЫКЛЮЧЕНЫ И ВКЛЮЧАЮТСЯ ЭТОЙ СТРОКОЙ ( ОНА ПИШЕТСЯ ) ВЫШЕ ВСЕГО КОДА!!!!!!!!!!!!!

// !Поэтому, если присутствует "use strict", то переменную всегда объявляем через let !!!!!



// !КЛЮЧЕВОЕ СЛОВО ДЛЯ ПЕРЕМЕННОЙ - let (раньше var)

// Если слово состоит из двух и более слов, принято использовать "Горбатый регистр" или CamelCase (LoverCamelCase)- пример StudentName

// let studentName; объявление переменной без значения

// а дальше можно присвоить
// studentName = "Игорь"  let  уже писать не нужно!!!!!!!


                                //    Переменные!!!!!!!!!!!!!

// Чтобы объявить несколько переменных сразу можно:

// let myName ="Masha" , myAge= "30", myCity ="Piter";
...
//                         ! Чтобы изменить имя переменной нужно:
...
// myAge = "18" стало
            ...
            // !Переменная myAge получает значение переменной myNewAge


                            //  ! Область видимости переменной

                            // Это блок -(функция)
        // !В рамках одного блока, переменную стоит использовать только после ее объявления


                                // ! const!!!!!!!!!!!!!!!!!
// Нельзя изменить тип данных константы


// !Имена констант с заранее известными значениями пишутся в верхнем регистре(заглавными)
// const BLOCK_SIZE = 25;  стандартный размер или код цвета
// const COLOR_RED = "#F00";
// !Имена констант со значениями присвоенными в процессе выполнения программы пишем также
// const summSizes = 25 + 30


// !Изменить можно ЗНАЧЕНИЕ ВНУТРИ ОПРЕДЕЛЕННЫХ  ТИПОВ !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
// Данные изменились без ошибок!

                                //   !КЛЮЧЕВОЕ СЛОВО VAR

// !1. ПЕРЕМЕННУЮ УЖЕ МОЖНО ИСПОЛЬЗОВАТЬ ДО ЕЕ ОБЪЯВЛЕНИЯ КЛЮЧЕВЫМ СЛОВОМ VAR
//   age = 35;
//   console.log(age);
  
//   var age;
//  ! 2. ЕСЛИ В КАЧЕСТВЕ БЛОКА ВЫСТУПАЕТ ФУНКЦИЯ, ТО VAR  ЗА ЕЕ ПРЕДЕЛАМИ ТАКЖЕ НЕ БУДЕТ ВИДЕТ, КАК И let  
//       но!!!!!!!!!
      
//       !3. ЕСЛИ  ЭТО УСЛОВИЕ ИЛИ ЦИКЛ, ТО VAR ,БУДЕТ ВИДНА!!
    
//       if (true) {
//         var sizeVar = 50;
//       }
//       console.log(sizeVar); переменная видна за пределами блока!!!!
